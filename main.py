...
# V14.2.3 雲端品牌校準穩定版：取消「用餐(填單人)」欄位 + 階梯式休息判定 + 20H 加班預警
...
def display_header():
    ...
    with col_title:
        st.title("化石先生：雲端工時分析系統 (V14.2.3)")
    st.markdown("---")

display_header()

def process_data_v14_2_3(file):
    try:
        file.seek(0)
...
if uploaded_file:
    ...
    if st.button("🚀 啟動衛星連線分析"):
        result = process_data_v14_2_3(uploaded_file)
                ...
                for month, data in month_dict.items():
                    ...
                    for r_idx, row in data.iterrows():
                        ...
                        for c_idx, col_n in enumerate(cols_d):
                            val = row[col_n]
                            fmt_d = {'border': 1, 'num_format': '0.0', 'align': 'left'}
                            if col_n == '人員':
                                fmt_d['font_color'] = c_sets['text']; fmt_d['bg_color'] = c_sets['bg']
                            elif is_off:
                                fmt_d['bg_color'] = '#FF0000'; fmt_d['font_color'] = '#000000'
                            else:
                                if col_n == '加班': fmt_d['font_color'] = '#FF0000'
                                elif col_n == '星期' and val in ['週六', '週日']: fmt_d['font_color'] = '#FF0000'; fmt_d['bold'] = True
                            ws_d.write(r_idx + 1, c_idx, val, wb.add_format(fmt_d))
                    ...
                    for r_idx, row_s in summary.iterrows():
                        ...
                        for c_idx, col_n in enumerate(summary.columns):
                            ...
                            if col_n == '人員':
                                ...
                                if val_s > 20.0: sum_fmt['bg_color'] = '#FF0000'; sum_fmt['font_color'] = '#FFFFFF'; sum_fmt['bold'] = True
                                else: sum_fmt['font_color'] = '#FF0000'
                            ws_s.write(r_idx + 1, c_idx, val_s, wb.add_format(sum_fmt))
                    ws_s.set_column('A:E', 15)

            st.download_button("📥 下載 V14.2.3 簡化校準報告", output_excel.getvalue(), "化石先生報告.xlsx")
