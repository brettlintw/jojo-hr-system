...
# V14.6.2 雲端品牌旗艦版：班次核對圖示化(綠/紅圓點) + 摘要術語精確化 + 異常診斷備註
...
def display_header():
    ...
    with col_logo:
        try:
            ...
            st.error("📷 Logo 遺失")
    with col_title:
        st.title("化石先生：雲端工時分析系統 (V14.6.2)")
    st.markdown("---")

display_header()

def process_data_v14_6_2(file):
    shift_rules = {
                    ...
                    for col_idx in range(2, len(dates_raw)):
                        ...
                        try:
                            ...
                            if shift != "nan" or work_h > 0:
                                ...
                                # --- V14.6.2 更新：班次核對圖示化 ---
                                check_status = "-" if is_off else "●" # 預設圓點
                                is_correct = False
                                if not is_off and shift in shift_rules:
                                    r_s, r_e = shift_rules[shift]
                                    if start_t == r_s and end_t == r_e: is_correct = True
                                ...
                                person_records.append({
                                    ...
                                    '出勤計算': 1 if (not is_off and work_h > 0) else 0, '_is_weekend': is_weekend, '_is_off': is_off, '_is_correct': is_correct
                                })
                        except: pass
...
if uploaded_file:
    ...
    if st.button("🚀 啟動衛星連線分析"):
        month_dict, shift_rules = process_data_v14_6_2(uploaded_file) 
        if not month_dict:
            st.error("❌ 檔案相容性異常。")
        else:
            ...
            with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
                ...
                for m_name, data in month_dict.items():
                    m_num_match = re.search(r'(\d+)', m_name); clean_m = m_num_match.group(1) if m_num_match else m_name
                ...
                ws_c.merge_range(0, 0, 0, 6, f"{rule_t}  |  檔名：{f_name_raw}  |  修改時間：{m_time}", info_f)
                ...
                for r_idx, row_c in total_confirm_df.iterrows():
                    is_we = row_c['平日/假日'] in ['週六', '週日']; is_m_end = (r_idx < len(total_confirm_df)-1) and (total_confirm_df.iloc[r_idx+1]['月份'] != row_c['月份'])
                    for c_idx, col_n in enumerate(total_confirm_df.columns):
                        val_c = row_c[col_n]; c_fmt = {'border': 1, 'align': 'left', 'valign': 'vcenter'}
                        if is_m_end: c_fmt['bottom'] = 5
                        if col_n == '平日/假日': c_fmt['font_color'] = '#FF0000' if is_we else '#008000'; val_c = row_c['平日/假日'] if is_we else "平日"
                        elif col_n == '當天人數': lim = (3, 4) if is_we else (2, 3); c_fmt['font_color'] = '#008000' if lim[0] <= val_c <= lim[1] else '#FF0000'
                        elif col_n == '排班人數確認': c_fmt['font_color'] = '#008000' if val_c == "正常" else '#FF0000'; c_fmt['bold'] = True
                        elif col_n == '備註' and row_c['排班人數確認'] == "異常": c_fmt['font_color'] = '#FF0000'
                        ws_c.write(r_idx + 2, c_idx, val_c, wb.add_format(c_fmt))
                ws_c.set_column('A:G', 15); ws_c.set_column('E:G', 40)
                ...
                for m_name, data in month_dict.items():
                    ...
                    ws = writer.sheets[safe_m]; ws.freeze_panes(2, 0)
                    ws.merge_range(0, 0, 0, start_col_sum + len(summary.columns) - 1, f"原始檔名：{f_name_raw}  |  修改時間：{m_time}", info_f)
                    ...
                    for r_idx, row in data.iterrows():
                        c_sets = p_color_map.get(row['人員']); is_bound = (r_idx == len(data)-1) or (data.iloc[r_idx+1]['人員'] != row['人員'])
                        # --- V14.6.2 更新：班次核對圓點顏色邏輯 ---
                        c_color = '#008000' if row['_is_correct'] else ('#FF0000' if not row['_is_off'] else '#000000')
                        
                        for c_idx, col_n in enumerate(cols_d):
                            val = row[col_n]; fmt = {'border': 1, 'align': 'left', 'valign': 'vcenter'}
                            ...
                            elif col_n == '班次核對': fmt['bold'] = True; fmt['font_color'] = c_color; fmt['align'] = 'center'; fmt['font_size'] = 14
                    ...
                    for r_idx, row_s in summary.iterrows():
                        ...
                        for c_idx, col_n in enumerate(summary.columns):
                            v_s = row_s[col_n]; s_fmt = {'border': 1, 'num_format': '0.0', 'align': 'left', 'valign': 'vcenter', 'bg_color': '#FFFFFF'}
